...
if [[ -e "${preflight_dir}" ]]; then
  echo "Refusing to overwrite preflight directory: ${preflight_dir}" >&2
  exit 2
fi
...
if [[ "${zero_stage}" == 3 ]]; then
  ...
  if [[ -e "${baseline_dir}" ]]; then
    echo "Refusing to overwrite preflight directory: ${baseline_dir}" >&2
    exit 2
  fi
...
if [[ -e "${resume_dir}" ]]; then
  echo "Refusing to overwrite resume preflight directory: ${resume_dir}" >&2
  exit 2
fi
...
if [[ -e "${output_dir}" ]]; then
  echo "Refusing to overwrite full training directory: ${output_dir}" >&2
  exit 2
fi
